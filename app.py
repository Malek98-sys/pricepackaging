...
def load_data(file_source=None, file_name: str = "") -> pd.DataFrame:
    ...
    if df is None:
        # On distingue le cas "moteur manquant" (xlrd absent) des autres
        _missing = any("Missing optional dependency" in e or "xlrd" in e.lower()
                       for e in errors)
        if real_kind == "xls" and _missing:
            raise ValueError(
                "Votre fichier est un ancien format Excel (.xls) qui nécessite la "
                "bibliothèque 'xlrd'. Deux solutions :\n"
                "  • Recommandé : ouvrez le fichier dans Excel → Enregistrer sous "
                "→ « Classeur Excel (*.xlsx) », puis ré-importez.\n"
                "  • Ou installez xlrd (ajoutez 'xlrd' au requirements.txt).\n"
                f"Détails : {' | '.join(errors[:3])}"
            )
        raise ValueError(
            "Impossible de lire ce fichier. Vérifiez qu'il s'agit bien d'un "
            "fichier Excel .xlsx valide (et non d'un fichier corrompu). "
            "Conseil : ouvrez-le dans Excel puis « Enregistrer sous » → "
            "« Classeur Excel (*.xlsx) », puis ré-importez-le.\n"
            f"Détails techniques : {' | '.join(errors[:3])}"
        )

    df.columns = df.columns.str.strip()

    # ── Compatibilité : sur les versions récentes de pandas (backend PyArrow),
    # certaines opérations .str.contains(regex) échouent. On reconvertit les
    # colonnes texte en type objet classique pour un comportement homogène
    # quel que soit l'environnement (PC local OU Streamlit Cloud).
    try:
        for _c in df.columns:
            if df[_c].dtype == "object" or str(df[_c].dtype).startswith("string"):
    ...
    if currency_col is None:
        ...
        for col in df.columns:
            ...
            if matches >= max(1, int(len(uniq) * 0.25)):
                currency_col = col
                break
            if vals.astype("object").str.contains(r'[€$£¥₹₩]', regex=True, na=False).any():
                currency_col = col
                break
